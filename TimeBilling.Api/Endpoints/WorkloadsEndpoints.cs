...
public static class WorkloadsEndpoints
{
    public static IEndpointRouteBuilder AddWorkloadsEndpoints(this IEndpointRouteBuilder builder)
    {
        ...
        _ = group.MapPost("/BeginWorkload",
                async Task<Results<Ok<WorkloadResponse>, NotFound>> ([FromBody] BeginWorkloadCommand workload, [FromServices] IMediator mediator) =>
                {
            ...
            .WithName("BeginWorkload")
            .WithOpenApi()
            .AddEndpointFilter<TimeBillingFeatureAFilter>()
            .Produces(StatusCodes.Status404NotFound)
            .Produces<WorkloadResponse>(StatusCodes.Status200OK);

        _ = group.MapPut("/EndWorkload",
                async Task<Results<Ok<WorkloadResponse>, NotFound>> ([FromBody] EndWorkloadCommand workload, [FromServices] IMediator mediator) =>
                {
                    WorkloadResponse? result = await mediator.Send(workload);
                    return result is null
                        ? TypedResults.NotFound()
